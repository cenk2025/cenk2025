            ...
            <div className="container mx-auto px-6">
                {!fullPage && (
                    <div className="text-center mb-12">
                        ...
                        <p className="text-lg text-gray-400 max-w-2xl mx-auto">Emme vain puhu tuloksista – me luomme niitä. Katso, miten olemme auttaneet asiakkaitamme menestymään.</p>
                    </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {itemsToShow.map((item, index) => (
                        <div 
                            key={item.id}
                            className={`group relative overflow-hidden rounded-2xl border border-gray-800 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
