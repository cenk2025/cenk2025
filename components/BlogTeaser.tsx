            ...
            <div className="container mx-auto px-6">
                 {!fullPage && (
                    <div className="text-center mb-12">
                        ...
                        <p className="text-lg text-gray-400 max-w-2xl mx-auto">Pysy kärryillä markkinoinnin uusimmista tuulista asiantuntijoidemme blogista.</p>
                    </div>
                 )}
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {posts.map((post, index) => (
                        <div 
                           key={post.id} 
                           className={`bg-dark-card rounded-2xl overflow-hidden border border-gray-800 flex flex-col transition-all duration-500 hover:border-brand-purple hover:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
